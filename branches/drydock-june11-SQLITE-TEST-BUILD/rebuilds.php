<?php
 /*
    ...
    */
	...
	{
		$db=new ThornDBI();
		$boardsquery = "SELECT * FROM ".THboards_table." WHERE hidden != 1 order by folder asc";
		$showcount = "SELECT COUNT(*) FROM ".THboards_table." WHERE hidden != 1";
		$showcount=$db->myresult($showcount);
		$boards=array();
	...
	{
		$db=new ThornDBI();
		$boardsquery = "SELECT * FROM ".THboards_table." WHERE hidden = 0 order by folder asc";
		$showcount = "SELECT COUNT(*) FROM ".THboards_table." WHERE hidden = 0";
		$showcount=$db->myresult($showcount);	
		$boards=array();
		$looper = 1;
		$queryresult=$db->myquery($boardsquery);
		...
		foreach($boards as $boardentry)
		{
			...
			if ($looper < $showcount ) { fwrite($sidelinks, '/'); };
			$looper++;
		}
    ...
    function rebuild_spamlist()
    {
        ...
        if($fp_cache)
        {
            ...
            foreach($bannedwords as $word)
            {
				...
				if($word!=null){fprintf($fp_cache, '"'.addslashes(rtrim($word))."\",\n");}     
            }
