...
{if $page-1!=$thispg}[<a href="{$THurl}{$binfo.folder}/{$page-1}">{$page-1}</a>]{else}[{$page-1}]{/if}
{/if}{/foreach}{if $page!=$thispg}[<a href="{$THurl}{$binfo.folder}/{$page}">{$page}</a>]{else}[{$page}]{/if}
</td><td>
{if $thispg!=$page}<a href="{$THurl}{$binfo.folder}/{$thispg+1}">Next</a>{else}Next{/if}
</td></tr></tbody></table>
